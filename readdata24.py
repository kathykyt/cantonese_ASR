...
from general_function.file_mp3 import *
...
class DataSpeech():
	...
	def __init__(self, path, type, LoadToMem = False, MemWavCount = 10000):
		...
		self.dic_wavlist_zh_HK = {}
		self.dic_symbollist_zh_HK = {}
	...
	def LoadDataList(self):
		...
		if(self.type=='train'):
			...
			filename_wavlist_zh_HK = 'zh-HK' + self.slash + 'train.wav.txt'
			...
			filename_symbollist_zh_HK = 'zh-HK' + self.slash + 'train.syllable.txt'
			
		elif(self.type=='dev'):
			#filename_wavlist_thchs30 = 'thchs30' + self.slash + 'cv.wav.lst'
			#filename_wavlist_stcmds = 'st-cmds' + self.slash + 'dev.wav.txt'
			filename_wavlist_zh_HK = 'zh-HK' + self.slash + 'dev.wav.txt'
			...
			filename_symbollist_zh_HK = 'zh-HK' + self.slash + 'dev.syllable.txt'
			
		elif(self.type=='test'):
			#filename_wavlist_thchs30 = 'thchs30' + self.slash + 'test.wav.lst'
			#filename_wavlist_stcmds = 'st-cmds' + self.slash + 'test.wav.txt'
			filename_wavlist_zh_HK = 'zh-HK' + self.slash + 'test.wav.txt'
			...
			filename_symbollist_zh_HK = 'zh-HK' + self.slash + 'test.syllable.txt'
		else:
		...
		self.dic_wavlist_zh_HK,self.list_wavnum_zh_HK = get_wav_list(self.datapath + filename_wavlist_zh_HK)
		...
		self.dic_symbollist_zh_HK,self.list_symbolnum_zh_HK = get_wav_symbol(self.datapath + filename_symbollist_zh_HK)
	...
	def GetDataNum(self):
		...
		num_wavlist_zh_HK = len(self.dic_wavlist_zh_HK)
		num_symbollist_zh_HK = len(self.dic_symbollist_zh_HK)
	...
	def GetData(self,n_start,n_amount=1):
		...
		#	reminder = n_start % bili
		#	length=len(self.list_wavnum_stcmds)
		#	filename = self.dic_wavlist_stcmds[self.list_wavnum_stcmds[(n + reminder - 1)%length]]
		#	list_symbol=self.dic_symbollist_stcmds[self.list_symbolnum_stcmds[(n + reminder - 1)%length]]
		
		if(n_start % bili == 0):
		        filename = self.dic_wavlist_zh_HK[self.list_wavnum_zh_HK[n_start // bili]]
		        list_symbol=self.dic_symbollist_zh_HK[self.list_symbolnum_zh_HK[n_start // bili]]
		else:
			n = n_start // bili * (bili - 1)
			reminder = n_start % bili
			length=len(self.list_wavnum_zh_HK)
			filename = self.dic_wavlist_zh_HK[self.list_wavnum_zh_HK[(n + reminder - 1)%length]]
			list_symbol=self.dic_symbollist_zh_HK[self.list_symbolnum_zh_HK[(n + reminder - 1)%length]]
				
		
		if('Windows' == plat.system()):
			filename = filename.replace('/','\\') # windows系统下需要执行这一行，对文件路径做特别处理
		
		# Need to convert mp3 to wav file first.
		WavFile =  mp3ToWav(self.datapath + filename);
